...
export default function JugglerGame() {
    ...
    const pullLever = () => {
        ...
        setWinHighlights([]); // Clear previous highlights
        // Gogo lamp OFF unless Bonus Flag or Gogo Chance Mode is active
        if (!stateRef.current.bonusFlag && !stateRef.current.gogoChanceMode) setGogoState('OFF');
    ...
    const stopReel = (idx) => {
        ...
        // Helper to find symbol distance in next 'maxFrames' frames
        const findCleanSymbolInSlip = (reelId, targetSyms, startIdx, avoidSyms = [], maxFrames = 4, offset = 0) => {
            ...
            for (let i = 0; i <= maxFrames; i++) {
                ...
                if (targetSyms.includes(symbol)) {
                    // Check Neighbors for Avoid Symbols relative to Visible Window
