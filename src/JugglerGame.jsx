...
const ODDS = {
    1: { bb: '1/273.1', rb: '1/439.8', grape: '1/6.5', cherry: '1/33.0', replay: '1/7.3', bell: '1/1092.3', bar: '1/1024.0', juggler: '1/1024.0', total: '1/168.5', payout: '97.0%' },
    2: { bb: '1/269.7', rb: '1/399.6', grape: '1/6.4', cherry: '1/33.0', replay: '1/7.3', bell: '1/1092.3', bar: '1/1024.0', juggler: '1/1024.0', total: '1/161.0', payout: '98.0%' },
    3: { bb: '1/269.7', rb: '1/331.0', grape: '1/6.3', cherry: '1/33.0', replay: '1/7.3', bell: '1/1092.3', bar: '1/1024.0', juggler: '1/1024.0', total: '1/148.6', payout: '99.5%' },
    4: { bb: '1/259.0', rb: '1/315.1', grape: '1/6.2', cherry: '1/33.0', replay: '1/7.3', bell: '1/1092.3', bar: '1/1024.0', juggler: '1/1024.0', total: '1/142.2', payout: '101.1%' },
    5: { bb: '1/259.0', rb: '1/255.0', grape: '1/6.1', cherry: '1/33.0', replay: '1/7.3', bell: '1/1092.3', bar: '1/1024.0', juggler: '1/1024.0', total: '1/128.5', payout: '103.3%' },
    6: { bb: '1/255.0', rb: '1/255.0', grape: '1/6.0', cherry: '1/33.0', replay: '1/7.3', bell: '1/1092.3', bar: '1/1024.0', juggler: '1/1024.0', total: '1/127.5', payout: '105.5%' },
};
...
export default function JugglerGame() {
    ...
    const pullLever = () => {
        ...
        if (stateRef.current.bonusStage) {
            ...
            const pBell = parseRatio(odds.bell);
            ...
            const tBell = tReplay + pBell;
            const tBar = tBell + pBar;
...
// --- PICK WINNER LINE (Randomize based on Bet) ---
if (command !== 'MISS') {
    const evalBet = stateRef.current.isReplay ? 3 : stateRef.current.bet;
    // Bet 1: Line 0 (Center)
    // Bet 2: Lines 0, 1, 2 (Center, Top, Bottom)
    // Bet 3: Lines 0..4 (All)
    // Bonus Stage: Treat as Max Bet (or at least 3 lines) for variety, or trust bet? 
    // User wants variety. Let's use Max Bet logic if in BonusStage for visual variety, 
    // BUT strict Juggler rules say you must bet to win line.
    // If user bets 1 and gets Bonus, it MUST align on Center.
    // So we strictly follow `evalBet`.

    let possibleLines = [0];
    if (evalBet >= 2) possibleLines = [0, 1, 2];
    if (evalBet >= 3) possibleLines = [0, 1, 2, 3, 4];

    // Filter lines for Bonus specific logic?
    // BB/RB can align on lines? Yes.
    // Grape/Cherry? Yes.

    const picked = possibleLines[Math.floor(Math.random() * possibleLines.length)];
    setWinLine(picked);
} else {
    setWinLine(0); // Default
}
...
const stopReel = (idx) => {
    ...
    // Visual Offsets for Lines: Center(0), Top(-1), Bottom(1), CrossDown, CrossUp
    // Reel 0, 1, 2 offsets
    const LINE_OFFSETS = [
        [0, 0, 0],       // 0: Center
        [-1, -1, -1],    // 1: Top
    ...
    const targetOffset = LINE_OFFSETS[lineIdx][idx];

    // Helper to find symbol distance in next 'maxFrames' frames (Prioritize Avoiding specific neighbors like Cherry)
    const findCleanSymbolInSlip = (reelId, targetSyms, startIdx, avoidSyms = [], maxFrames = 4, offset = 0) => {
        ...
        for (let i = 0; i <= maxFrames; i++) {
            ...
            // We want the symbol at (idx + offset) to be the target
            const checkPos = (idx + offset + 21) % 21;
            const symbol = strip[checkPos];

            if (targetSyms.includes(symbol)) {
                // Check Neighbors for Avoid Symbols (Top and Bottom OF THE VISIBLE WINDOW)
                // Visible window is relative to `idx` (Center).
                // Top is `idx - 1`, Bottom is `idx + 1`.
                // We need to ensure neither contains 'avoidSyms'.
    ...
    if (cmd === 'GRAPE') {
        const found = findCleanSymbolInSlip(idx, ['🍇'], naturalIdx, ['🍒'], 4, targetOffset);
        if (found !== null) finalIdx = found;
    } else if (cmd === 'CHERRY' && idx === 0) {
        // Cherry always aligns based on valid lines? 
        // Actually Cherry only needs to appear in Top/Center/Bottom. It pays scatter style (any column), but usually Left Reel.
        // Line logic applies? 
        // If WinLine is Center, we prefer Center.
        // Let's use targetOffset.
        const found = findCleanSymbolInSlip(idx, ['🍒'], naturalIdx, [], 4, targetOffset); // Don't avoid logic for cherry itself
        if (found !== null) finalIdx = found;
    } else if (cmd === 'BELL') {
        const found = findCleanSymbolInSlip(idx, ['🔔'], naturalIdx, ['🍒'], 4, targetOffset);
        if (found !== null) finalIdx = found;
    } else if (cmd === 'BAR_WIN') {
        const found = findCleanSymbolInSlip(idx, ['BAR'], naturalIdx, ['🍒'], 4, targetOffset);
        if (found !== null) finalIdx = found;
    } else if (cmd === 'JUGGLER') {
        const found = findCleanSymbolInSlip(idx, ['🤡'], naturalIdx, ['🍒'], 4, targetOffset);
        if (found !== null) finalIdx = found;
        ...
        const found = findCleanSymbolInSlip(idx, target, naturalIdx, ['🍒'], 21, targetOffset);
        if (found !== null) finalIdx = found;
    } else {
...
useEffect(() => {
    if (!spinning.some(s => s) && isPlaying) {
        ...
        for (const { offsets, symbols } of activeLineDefs) {
            ...
            else if (s1 === '🔔' && s2 === '🔔' && s3 === '🔔') { totalWin += 14; lineWin = true; } // Bell Win (14 Credits)
